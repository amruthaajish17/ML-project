...
In this project we used specific versions of the libraries. Save the following libraries in a text file- requirements.txt. Install all these libraries using the code py -r pip install requirements.txt.
...
sns.heatmap(iris_num.corr(), cmap='hot', annot = True);
```
...
For this project, we will use the LogisticRegression, a robust and versatile model that performs well with the Iris dataset.
